...
+(void)setWriteJsonDic:(NSDictionary *)dic toPath:(NSString *)path
{
    ...
    if (!jsonData || error) {
       // NSLog(@"过滤文字JSON解码失败"));
       // NSLog(@"过滤文字JSON文件%@ 写入失败 error-- %@"),path,error);
       } else {
...
+(id)getJsonDataForPath:(NSString *)path
{
    ...
    NSError *error;
    if (!jsonData) {
       // NSLog(@"过滤文字JSON文件%@ 解码失败 error--"),path);
        return nil;
    } else {
