...
@property(nonatomic,strong)TiUIMeiZhuangView *meizhuangView;
...
-(TiUIMeiZhuangView *)meizhuangView{
    if (_meizhuangView==nil) {
        _meizhuangView =[[TiUIMeiZhuangView alloc]init];
        WeakSelf;
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section{
    switch (self.mode.menuTag) {
        ...
        case 1:
          ...
          break;
            case 12:
              return [TIMenuPlistManager shareManager].meizhuangModArr.count;
            break;
        default:
            return 0;
           break;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
   ...
   TiUISubMenuOneViewCell * cell = [collectionView dequeueReusableCellWithReuseIdentifier:TiUIMenuCollectionViewCellId forIndexPath:indexPath];
    ...
    switch (self.mode.menuTag) {
        ...
        case 12:
                {
                 TIMenuMode * subMod = [[TIMenuPlistManager shareManager].meizhuangModArr objectAtIndex:indexPath.row];
                    if (subMod.selected)
                        {
                          self.selectedIndexPath = [NSIndexPath indexPathForRow:indexPath.row inSection:0];
                        }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.mode.menuTag ==12) {
        ...
        self.meizhuangView.mode = [[TIMenuPlistManager shareManager].meizhuangModArr objectAtIndex:indexPath.row];
...
- (void)setMode:(TIMenuMode *)mode{
    if (mode) {
       _mode = mode;
    [self.totalSwitch setTitle:[NSString stringWithFormat:kLocalizationMsg(@"%@:关"),mode.name] withImage:[UIImage imageNamed:@"btn_close"] withTextColor:TI_Color_Default_Text_Black forState:UIControlStateNormal];
    [self.totalSwitch setTitle:[NSString stringWithFormat:kLocalizationMsg(@"%@:开"),mode.name] withImage:[UIImage imageNamed:@"btn_open"] withTextColor:TI_Color_Default_Background_Pink forState:UIControlStateSelected];
