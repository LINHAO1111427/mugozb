    ...
    [self.databaseQueue inDatabase:^(FMDatabase *db) {
        ...
        if([db open]){
            ...
            if (![localVerson isEqualToString:preVersion]) {
                ...
                if (!result) {
                   // NSLog(@"过滤文字delete database faliure"));
                }
            }
            ...
            if (result) {
               ...
               // NSLog(@"过滤文字create database table failure"));
                NSError *error = [NSError errorWithDomain:@"com.qusdk" code:0 userInfo:@{@"errorInfo":kLocalizationMsg(@"数据库创建失败")}];
                if (failure) {
                    failure(error);
                }
            }
        } else {
           // NSLog(@"过滤文字open database faliure"));
            NSError *error = [NSError errorWithDomain:@"com.qusdk" code:0 userInfo:@{@"errorInfo":kLocalizationMsg(@"数据库打开失败")}];
            if (failure) {
                failure(error);
            }
        }
...
- (void)closeDB {
    ...
    [self.databaseQueue inDatabase:^(FMDatabase *db) {
        ...
        if (result) {
           // NSLog(@"过滤文字close databse success"));
        } else {
           // NSLog(@"过滤文字close databse faliure"));
        }
    }];
}
...
- (void)insertDataWithEffectResourceModel:(AliyunEffectResourceModel *)model {
    ...
    [self.databaseQueue inDatabase:^(FMDatabase *db) {
        ...
        if (!result) {
           // NSLog(@"过滤文字insert database error"));
        }
    }];
}
...
- (void)updateDataWithEffectResourceModel:(AliyunEffectResourceModel *)model {
    ...
    [self.databaseQueue inDatabase:^(FMDatabase *db) {
        ...
        if (!result) {
           // NSLog(@"过滤文字update database error"));
        }
    }];
