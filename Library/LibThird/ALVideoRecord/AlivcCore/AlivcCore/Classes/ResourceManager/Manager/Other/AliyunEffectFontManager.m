...
- (NSString *)registerFontWithFontPath:(NSString *)fontPath {
    ...
    if (!dynamicFontData) {
        ...
        dynamicFontData = [NSData dataWithContentsOfFile:fontPathFormat];
       // NSLog(@"过滤文字font path rename befor:%@"),fontPath);
       // NSLog(@"过滤文字font path renamed:%@"),fontPathFormat);
    }
    if (!dynamicFontData) {
       // NSLog(@"过滤文字font data read error:%@"), fontPath);
        return nil;
    }
...
//    }
    ...
    if (CTFontManagerRegisterFontsForURL((__bridge CFURLRef)fontUrl,kCTFontManagerScopeProcess,&error)) {
        ...
        if (!keepFontRegister) {
           // NSLog(@"过滤文字font register not success: can not get font call [UIFont fontWithName:size:]"));
            CFRelease(font);
        ...
        NSInteger errorCode = CFErrorGetCode(error);//105 表示已经注册过
           // NSLog(@"过滤文字errorcode == %zd"),errorCode);
   ...
   // NSLog(@"过滤文字font:%@ register success"), fontName);
    CFRelease(font);
    CFRelease(cfFontName);
    CFRelease(providerRef);
