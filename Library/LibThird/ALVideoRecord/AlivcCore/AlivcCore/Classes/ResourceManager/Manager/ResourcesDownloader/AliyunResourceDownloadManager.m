...
#import <AliyunVideoSDKBasic/AliyunHttpClient.h>
...
- (void)downloadResource {
    ...
    if (type == AliyunEffectTypeMV) {
        ...
        // 其他
        [self completionOnMainQueue:nil error:[NSError errorWithDomain:@"com.duanqu.download" code:-1000 userInfo:@{@"errorInfo":kLocalizationMsg(@"非法的素材类型")}]];
    }
}
...
- (void)unZipFrom:(NSString *)filePath to:(NSString *)toDestination {
    ...
    if (!toDestination) {
        ...
        // 解压失败
        if (!unzip) {
            NSError *error = [NSError errorWithDomain:@"com.duanqu" code:-1000 userInfo:@{@"errorInfo":kLocalizationMsg(@"解压失败")}];
            [self completionOnMainQueue:nil error:error];
            return ;
        }
        ...
        if (_task.resourceModel.effectType == AliyunEffectTypeFont) {
            ...
            if (!fontName) {
                NSError *error = [NSError errorWithDomain:@"com.duanqu.download" code:-1000 userInfo:@{@"errorInfo":kLocalizationMsg(@"字体注册失败")}];
                [self completionOnMainQueue:nil error:error];
                return;
            }
