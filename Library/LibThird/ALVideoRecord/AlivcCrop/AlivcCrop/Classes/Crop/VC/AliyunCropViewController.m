...
#import <AliyunVideoSDKBasic/AliyunCrop.h>
#import <AliyunVideoSDKBasic/AliyunImageCrop.h>
...
- (void)cropTaskOnComplete {
   ...
   // NSLog(@"过滤文字TestLog, %@:%@"), @"log_crop_complete_time", @([NSDate date].timeIntervalSince1970));
    self.progressView.progress = 0;
    if (_isCancel) {
        _isCancel = NO;
    } else {
...
//            _cutPanel.fadeDuration = 0;
           ...
           // NSLog(@"过滤文字裁剪编码方式：%d"),_cutPanel.encodeMode);
            _cutPanel.fillBackgroundColor = _cutInfo.backgroundColor;
            _cutPanel.useHW = _cutInfo.gpuCrop;
            
           // NSLog(@"过滤文字TestLog, %@:%@"), @"log_crop_start_time", @([NSDate date].timeIntervalSince1970));
...
- (void)applicationDidEnterBackground:(NSNotification *)notification {
   // NSLog(@"过滤文字applicationDidEnterBackground"));
    [self didStopCut];
    [self pauseVideo];
}
...
- (void)dealloc {
   // NSLog(@"过滤文字dealloc-------->AliyunCropViewController"));
    if (!_cutInfo.phAsset) {
        if (_timeObserver) {
            [_avPlayer removeTimeObserver:_timeObserver];
        }
