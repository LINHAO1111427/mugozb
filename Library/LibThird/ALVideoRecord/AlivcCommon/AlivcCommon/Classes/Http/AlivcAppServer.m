...
+ (void)getWithUrlString:(NSString *)urlString
    ...
    [[self manager] GET:string parameters:@{} headers:nil progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        if ([responseObject isKindOfClass:[NSDictionary class]]) {
            ...
            handle(kLocalizationMsg(@"数据格式异常"),nil);
        }
...
+ (void)getStsDataSucess:(void (^)(NSString * _Nonnull, NSString * _Nonnull, NSString * _Nonnull))sucess failure:(void (^)(NSString * _Nonnull))failure {
    ...
    [[self manager] GET:urlString parameters:nil headers:nil progress:^(NSProgress * _Nonnull downloadProgress) {
         
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSDictionary *resultDic = responseObject[@"data"];
       // NSLog(@"过滤文字%@"),resultDic);
        //AccessKeyId
        ...
        if (keyIDString && accessKeySecret && securityToken) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                failure([kLocalizationMsg(@"数据解析错误") localString]);
            });
        }
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
        if (error.code == -1009) {
            failure([kLocalizationMsg(@"当前无网络,请检查网络连接") localString]);
        }else{
        ...
        if (!error) {
            ...
            if (data) {
               ...
               // NSLog(@"过滤文字%@"),resultDic);
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    failure([kLocalizationMsg(@"返回数据为空") localString]);
                });
