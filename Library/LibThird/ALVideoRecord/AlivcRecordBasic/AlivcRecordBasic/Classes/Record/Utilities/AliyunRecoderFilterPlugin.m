...
- (instancetype)initWithFilterArry:(NSArray *)filterArry{
    ...
    if (self) {
        ...
        [self.filterArry addObject:NSLocalizedString(@"原来" , nil)];
...
- (void)displayFilterWithIndex:(int)index
{
    ...
    if (index == 0) {
        [self displayFilterName:NSLocalizedString(@"原色" , nil)];
    } else {
...
- (void)displayFilterName:(NSString *)filterName {
    ...
    [self.filterLabel.layer addAnimation:keyframeAnimation forKey:@"opacity"];
     ...
     self.filterLabel.text =  NSLocalizedString(filterName, nil) ;
}
...
-(BOOL)isRecording{
    if (self.delegate && [self.delegate respondsToSelector:@selector(isRecordingDelegate)]) {
        return [self.delegate isRecordingDelegate];
    }
    return NO;
}
