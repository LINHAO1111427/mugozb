...
+ (void)showInfoWithStatus:(NSString*)status {
    if ([status isKindOfClass:[NSNull class]] || status.length == 0 || [status isEqualToString:@"<null>"]) {
        return;
    }
...
+ (void)showErrorWithStatus:(NSString*)status {
    if ([status isKindOfClass:[NSNull class]] || status.length == 0) {
        return;
    }
...
+ (void)showGifLoadingWithtatus:(nullable NSString*)status{
    NSString *path = [[NSBundle mainBundle] pathForResource:@"LibToolsB" ofType:@"bundle"];
    NSBundle *imageBundle = [NSBundle bundleWithPath:path];
    NSString *imagePath = [imageBundle pathForResource:@"info_loading" ofType:@"gif"];
    NSData *imageData = [NSData dataWithContentsOfFile:imagePath];
    UIImage *gif = [UIImage imageGifWithData:imageData];
    NSTimeInterval displayInterval = [self displayDurationForString:status];
    SVProgressHUD *instance = [SVProgressHUD sharedView];
    instance.defaultStyle = SVProgressHUDStyleCustom;
    instance.backgroundColor = [UIColor colorWithWhite:0 alpha:0.0];;
    instance.backgroundLayerColor = [UIColor colorWithWhite:0 alpha:0.0];
    instance.foregroundColor = [UIColor colorWithWhite:0 alpha:0.0];
     
    instance.imageViewSize = CGSizeMake(80, 80);
    [instance showImage:gif status:status duration:displayInterval];
