...
#import "HttpApiApiRechargeRules.h"




@implementation HttpApiApiRechargeRules
...
{
NSString *strUrl=[NSString stringWithFormat:@"%@%@",[HttpClient getBaseUrl],@"/api/rechargeRules/rulesList",nil];
...
[HttpClient requestPostWithPath:strUrl param:dictPara success:^(id dataBody) {
        ...
        RechargeCenterVOModel *retModel;
        if(code==1)
        {
            id retObj= dicRet[@"retObj"];
            if([retObj isKindOfClass:[NSDictionary class]])
            {
                retModel=   [RechargeCenterVOModel getFromDict:retObj];
            }
        }
