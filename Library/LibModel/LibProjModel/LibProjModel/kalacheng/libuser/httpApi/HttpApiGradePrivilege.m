...
#import "HttpApiGradePrivilege.h"




@implementation HttpApiGradePrivilege
...
{
NSString *strUrl=[NSString stringWithFormat:@"%@%@",[HttpClient getBaseUrl],@"/api/h5/privlilegeInfo",nil];
...
[HttpClient requestPostWithPath:strUrl param:dictPara success:^(id dataBody) {
        ...
        AppUserPrivilegeDTOModel *retModel;
        if(code==1)
        {
            id retObj= dicRet[@"retObj"];
            if([retObj isKindOfClass:[NSDictionary class]])
            {
                retModel=   [AppUserPrivilegeDTOModel getFromDict:retObj];
            }
        }
