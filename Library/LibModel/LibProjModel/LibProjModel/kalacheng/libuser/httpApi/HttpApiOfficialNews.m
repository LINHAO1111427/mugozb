...
#import "HttpApiOfficialNews.h"




@implementation HttpApiOfficialNews
...
{
NSString *strUrl=[NSString stringWithFormat:@"%@%@",[HttpClient getBaseUrl],@"/api/officialNews/getOfficialNewsList",nil];
        ...
        if(code==1)
        {
            ...
            if([idArray isKindOfClass:[NSArray class]])
            {
                ...
                for (NSDictionary *dic in dataArray) {
                    AppOfficialNewsDTOModel *handle = [AppOfficialNewsDTOModel getFromDict:dic];
...
{
NSString *strUrl=[NSString stringWithFormat:@"%@%@",[HttpClient getBaseUrl],@"/api/officialNews/messageViewed",nil];
NSDictionary *dictPara=@{@"_uid_":@([HttpClient getUid]) ,@"_token_":[HttpClient getToken],@"_OS_":[HttpClient getOSType],@"_OSV_":[HttpClient getOSVersion],@"_OSInfo_":[HttpClient getOSInfo]
,@"id":@(id_field)};
...
[HttpClient requestPostWithPath:strUrl param:dictPara success:^(id dataBody) {
        ...
        int code= [dicRet getInt:@"code"];
        NSString*strMsg= [dicRet getStr:@"msg"];
        AppOfficialNewsDTOModel *retModel;
        if(code==1)
        {
            id retObj= dicRet[@"retObj"];
            if([retObj isKindOfClass:[NSDictionary class]])
            {
                retModel=   [AppOfficialNewsDTOModel getFromDict:retObj];
            }
        }
