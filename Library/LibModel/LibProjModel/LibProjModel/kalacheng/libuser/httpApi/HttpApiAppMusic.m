...
#import "HttpApiAppMusic.h"




@implementation HttpApiAppMusic
...
{
NSString *strUrl=[NSString stringWithFormat:@"%@%@",[HttpClient getBaseUrl],@"/api/music/queryList",nil];
        ...
        if(code==1)
        {
            ...
            if([idArray isKindOfClass:[NSArray class]])
            {
                ...
                for (NSDictionary *dic in dataArray) {
                    AppMusicDTOModel *handle = [AppMusicDTOModel getFromDict:dic];
...
{
NSString *strUrl=[NSString stringWithFormat:@"%@%@",[HttpClient getBaseUrl],@"/api/music/uploadMusic",nil];
NSDictionary *dictPara=@{@"_uid_":@([HttpClient getUid]) ,@"_token_":[HttpClient getToken],@"_OS_":[HttpClient getOSType],@"_OSV_":[HttpClient getOSVersion],@"_OSInfo_":[HttpClient getOSInfo]
,@"author":author
,@"cover":cover
,@"musicUrl":musicUrl
...
{
NSString *strUrl=[NSString stringWithFormat:@"%@%@",[HttpClient getBaseUrl],@"/api/music/queryMyMusicList",nil];
NSDictionary *dictPara=@{@"_uid_":@([HttpClient getUid]) ,@"_token_":[HttpClient getToken],@"_OS_":[HttpClient getOSType],@"_OSV_":[HttpClient getOSVersion],@"_OSInfo_":[HttpClient getOSInfo]
,@"name":name
,@"pageIndex":@(pageIndex)
,@"pageSize":@(pageSize)};
 ...
 [HttpClient requestPostWithPath:strUrl param:dictPara success:^(id dataBody) {
        ...
        if(code==1)
        {
            ...
            if([idArray isKindOfClass:[NSArray class]])
            {
                NSArray*  dataArray=idArray;
                for (NSDictionary *dic in dataArray) {
                    AppUserMusicDTOModel *handle = [AppUserMusicDTOModel getFromDict:dic];
                    [retArr addObject:handle];
                }
            }
        }
...
{
NSString *strUrl=[NSString stringWithFormat:@"%@%@",[HttpClient getBaseUrl],@"/api/music/setMusic",nil];
NSDictionary *dictPara=@{@"_uid_":@([HttpClient getUid]) ,@"_token_":[HttpClient getToken],@"_OS_":[HttpClient getOSType],@"_OSV_":[HttpClient getOSVersion],@"_OSInfo_":[HttpClient getOSInfo]
,@"musicId":@(musicId)
,@"type":@(type)};
