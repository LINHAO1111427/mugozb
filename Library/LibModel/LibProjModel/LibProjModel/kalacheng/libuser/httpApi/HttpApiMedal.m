...
#import "HttpApiMedal.h"




@implementation HttpApiMedal
...
{
NSString *strUrl=[NSString stringWithFormat:@"%@%@",[HttpClient getBaseUrl],@"/api/medal/getMyAllMedal",nil];
...
[HttpClient requestPostWithPath:strUrl param:dictPara success:^(id dataBody) {
        ...
        MedalDtoModel *retModel;
        if(code==1)
        {
            id retObj= dicRet[@"retObj"];
            if([retObj isKindOfClass:[NSDictionary class]])
            {
                retModel=   [MedalDtoModel getFromDict:retObj];
            }
        }
