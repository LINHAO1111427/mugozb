...
#import "HttpApiRestShop.h"




@implementation HttpApiRestShop
...
{
NSString *strUrl=[NSString stringWithFormat:@"%@%@",[HttpClient getBaseUrl],@"/api/restshop/businessCenter",nil];
...
[HttpClient requestPostWithPath:strUrl param:dictPara success:^(id dataBody) {
        ...
        ShopBusinessDTOModel *retModel;
        if(code==1)
        {
            id retObj= dicRet[@"retObj"];
            if([retObj isKindOfClass:[NSDictionary class]])
            {
                retModel=   [ShopBusinessDTOModel getFromDict:retObj];
            }
        }
...
{
NSString *strUrl=[NSString stringWithFormat:@"%@%@",[HttpClient getBaseUrl],@"/api/restshop/searchBusinessProduct",nil];
NSDictionary *dictPara=@{@"_uid_":@([HttpClient getUid]) ,@"_token_":[HttpClient getToken],@"_OS_":[HttpClient getOSType],@"_OSV_":[HttpClient getOSVersion],@"_OSInfo_":[HttpClient getOSInfo]
,@"businessId":@(businessId)
