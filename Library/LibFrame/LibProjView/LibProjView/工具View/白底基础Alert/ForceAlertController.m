...
- (void)createUI{
    ...
    if (_optionArr.count) {
        ...
        for (int i = 0; i<_optionArr.count; i++) {
            ...
            [btn setTitleColor:_optionArr[i][@"titleColor"]?[self btnTextColor:[_optionArr[i][@"titleColor"] integerValue] index:i]:[UIColor blackColor] forState:UIControlStateNormal];
...
- (void)addOptions:(NSString *)option textColor:(ForceAlertBtnTextColor)color clickHandle:(void (^)(void))handle{
...
- (void)addOptions:(NSString *)option textUIColor:(UIColor *)color clickHandle:(void(^ __nullable)(void))handle{
    NSMutableDictionary *item = [[NSMutableDictionary alloc] initWithCapacity:1];
       [item setObject:option forKey:@"title"];
       if (color) {
           [item setObject:color forKey:@"titleUIColor"];
       }
       if (handle) {
           [item setObject:handle forKey:@"handle"];
       }
...
- (UIColor *)btnTextColor:(ForceAlertBtnTextColor)color index:(int)index{
    
    switch (color) {
        case ForceAlert_NormalColor:
            return [ProjConfig normalColors];
            break;
        case ForceAlert_BlackColor:
            return [UIColor blackColor];
            break;
        case ForceAlert_OrangeColor:
            return kRGB_COLOR(@"#FC8F3A");
            break;
        case ForceAlert_CustomColor:
            return _optionArr[index][@"titleUIColor"];
            break;
        default:
            return [UIColor blackColor];
            break;
