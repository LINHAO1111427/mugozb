...
- (void)setupToolbars
{
    ...
    indexLabel.bounds = CGRectMake(0, [UIScreen mainScreen].bounds.size.height - 40  , 80  , 30  );
...
//    [saveButton setTitle:kLocalizationMsg(@"保存") forState:UIControlStateNormal];
    [saveButton setImage:[UIImage imageNamed:@"comment_download"] forState:UIControlStateNormal];
    saveButton.imageEdgeInsets = UIEdgeInsetsMake(2, 13, 2, 13);
...
- (void)setupImageOfImageViewForIndex:(NSInteger)index
{
    ...
    if (index < _scrollView.subviews.count) {
        imageView = _scrollView.subviews[index];
...
-(void)delayMethod{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"keyboardTongzhi" object:nil userInfo:@{@"":@""}];
...
- (void)photoClick:(UITapGestureRecognizer *)recognizer
{
    ...
    if ([self.sourceImagesContainerView isKindOfClass:UICollectionView.class]) {
        ...
        sourceView.contentMode = UIViewContentModeScaleAspectFill;
    }
...
- (void)layoutSubviews
{
    ...
    _indexLabel.center = CGPointMake(self.bounds.size.width * 0.5, self.bounds.size.height -35  );
    _saveButton.frame = CGRectMake( [UIScreen mainScreen].bounds.size.width - 70  , 30  , 50  , 25  );
}
