...
//    [cell.iconImgView sd_setHighlightedImageWithURL:photo.url options:SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
...
//
//                if (error) {
//        //            [MBProgressHUD showError:kLocalizationMsg(@"加载图片失败.")];
//                   // NSLog(@"过滤文字error = %@"), error);
//                }
//    }];
    
    [cell.iconImgView sd_setImageWithURL:photo.url placeholderImage:photo.srcImageView.image options:SDWebImageRetryFailed];
