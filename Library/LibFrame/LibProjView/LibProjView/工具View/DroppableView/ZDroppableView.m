...
{
    //   // NSLog(@"过滤文字touchesEnded 3"));
    [self endDrag];
}

- (void)touchesCancelled:(NSSet *)touches withEvent:(UIEvent *)event
{
    //   // NSLog(@"过滤文字touchesCancelled 4"));
    [self endDrag];
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    //NSLog(@"过滤文字point:%@ : newPoint:%@"),NSStringFromCGPoint(point),NSStringFromCGPoint(newPoint));
    }
    return view;
}
