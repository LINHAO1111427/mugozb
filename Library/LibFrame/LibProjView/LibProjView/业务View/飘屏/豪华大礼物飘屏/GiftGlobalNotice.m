...
- (GiftGlobalBannerView *)bannerV{
    if (!_bannerV) {
        GiftGlobalBannerView *bgV = [[GiftGlobalBannerView alloc] initWithFrame:CGRectMake(_superV.size.width, liveGlobalGiftBannerY, _superV.size.width, 50)];
...
- (void)playNextGift{
    ...
    if (_itemArr.count) {
        ...
        [self.bannerV showGiftModel:giftModel];
        [self showBannerView];
    ...
    if ([self chectShowData:self.bannerV.giftModel newGiftData:model]) {
        {
            ...
            [self reStartTime];
            [_bannerV changeLiansongNumber:self.bannerV.giftModel.continuousNumber];
        }
