...
@implementation GiftFansShowObj
...
- (GiftBannerView *)bannerV{
    if (!_bannerV) {
        GiftBannerView *bgV = [[GiftBannerView alloc] initWithFrame:CGRectMake(-_superV.size.width, liveFansGiftBannerY, _superV.size.width, 50)];
...
- (void)playNextGift{
    ...
    if (_itemArr.count) {
        ...
        [self.bannerV showView:GiftBannerForFans giftModel:giftModel];
        [self showBannerView];
