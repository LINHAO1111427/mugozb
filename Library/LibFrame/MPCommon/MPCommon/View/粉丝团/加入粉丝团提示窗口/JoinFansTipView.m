...
@interface JoinFansTipView ()

@end

@implementation JoinFansTipView

+ (void)showJoinFansTip:(UIView *)superV {
    if ([LiveManager liveInfo].liveType != LiveTypeForOneToOne) {   ///多人视频
        BOOL hasSelf = NO;
        for (UIView *subV in superV.subviews) {
            if ([subV isKindOfClass:[JoinFansTipView class]]) {
                hasSelf = YES;
            }
        }
        if (!hasSelf && [LiveManager liveInfo].roomModel.isFans == 0 && [LiveManager liveInfo].anchorId != [KLCUserInfo getUserId]) {
            
            [HttpApiAnchorController liveFansTeamInfo:[LiveManager liveInfo].anchorId callback:^(int code, NSString *strMsg, FansInfoDtoModel *model) {
                if (code == 1 && model.isMember == 0) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        JoinFansTipView *tip = [[JoinFansTipView alloc] init];
                        [superV addSubview:tip];
                        [tip createUI];
                    });
                }
            }];
...
- (void)createUI{
    ...
    [attentionBtn setTitle:kLocalizationMsg(@"马上加入") forState:UIControlStateNormal];
    ...
    self.frame = CGRectMake(12, 0, kScreenWidth-24, 80);
