...
+ (void)showInSuperView:(UIView *)superV callBack:(OrderMoreSelectedBlock)callBack{
    ...
    selectedView.layer.borderColor = kRGBA_COLOR(@"#DDDDDD", 0.4).CGColor;
    ...
    NSArray *titles = @[kLocalizationMsg(@"全部"),kLocalizationMsg(@"淘宝订单"),kLocalizationMsg(@"京东订单"),kLocalizationMsg(@"拼多多订单"),kLocalizationMsg(@"有赞订单"),kLocalizationMsg(@"有赞订单"),kLocalizationMsg(@"官方小店订单")];
    for (int i = 0; i < titles.count; i++) {
        UIButton *btn = [[UIButton alloc]initWithFrame:CGRectMake(8, 10+42*i, 94, 40)];
        btn.tag = i;
        btn.contentHorizontalAlignment = UIControlContentHorizontalAlignmentLeft;
        btn.titleLabel.font = [UIFont systemFontOfSize:14];
        [btn setTitleColor:kRGB_COLOR(@"#555555") forState:UIControlStateNormal];
        ...
        line.backgroundColor = kRGB_COLOR(@"#DDDDDD");
        if (i != titles.count-1) {
