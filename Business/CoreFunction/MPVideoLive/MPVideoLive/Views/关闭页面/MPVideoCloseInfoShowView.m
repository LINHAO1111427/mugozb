...
#import "MPVideoCloseInfoShowView.h"
...
@interface MPVideoCloseInfoShowView ()<MPLiveCloseInterface>
...
@implementation MPVideoCloseInfoShowView
...
+ (void)showInView:(UIView *)superV isAnchor:(BOOL)isAnchor closeInfo:(ApiCloseLiveModel *)closeModel{
    ...
    for (UIView *subV in superV.subviews) {
        if ([subV isKindOfClass:[MPVideoCloseInfoShowView class]]) {
            hasView = YES;
        }
    }
    if (!hasView) {
        MPVideoCloseInfoShowView *showV = [[MPVideoCloseInfoShowView alloc] initWithFrame:superV.bounds];
...
- (void)createView:(BOOL)isAnchor{
    ...
    [anchorImg sd_setImageWithURL:[NSURL URLWithString:voModel.liveThumb] placeholderImage:[ProjConfig getDefaultImage]];
