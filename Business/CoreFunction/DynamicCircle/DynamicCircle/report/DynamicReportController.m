...
#import <LibProjModel/DynamicAppealTypeVOModel.h>

@interface DynamicReportController ()<UITableViewDelegate,UITableViewDataSource,UITextViewDelegate,ZDropScrollViewDelegate>
...
@implementation DynamicReportController
...
- (void)createUI{
    ...
    [table registerClass:[DynamicReportItemCell class] forCellReuseIdentifier:DynamicReportItemCellIdentifier];
...
- (void)requetData{
    kWeakSelf(self);
    [HttpApiDynamicController getDynamicAppealTypeList:^(int code, NSString *strMsg, NSArray<DynamicAppealTypeVOModel *> *arr) {
        if (code == 1) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    DynamicReportItemCell *cell = [tableView dequeueReusableCellWithIdentifier:DynamicReportItemCellIdentifier];
    DynamicAppealTypeVOModel *model =  _dataArr[indexPath.row];
...
- (void)repostCommit{
    ...
    if (self.dynamicId <= 0) {
        [SVProgressHUD showInfoWithStatus:kLocalizationMsg(@"请选择有效动态")];
        return;
    }
    ...
    DynamicAppealTypeVOModel *model =  _dataArr[self.selectIndex];
    kWeakSelf(self);
    [HttpApiDynamicController addDynamicAppeal:self.repostTextV.text dynamicAppealImages:picStr dynamicAppealTypeId:model.id_field dynamicAppealTypeName:model.appealTypeName dynamicId:self.dynamicId callback:^(int code, NSString *strMsg, HttpNoneModel *model) {
        if (code == 1) {
