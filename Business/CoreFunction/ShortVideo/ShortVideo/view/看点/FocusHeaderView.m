...
#import <LibProjView/AutoScrollLabel.h>
...
///文字滚动
@property(nonatomic,weak)AutoScrollLabel *labelBanner;
///广告背景
...
- (NewPagedFlowView *)pageFlowView{
    if (!_pageFlowView) {
        NewPagedFlowView *pageFlowView = [[NewPagedFlowView alloc] initWithFrame:CGRectMake(0, 5, kScreenWidth, 0.1)];
...
- (void)setAdbannerStr:(NSString *)adbannerStr{
    ...
    if (adbannerStr.length > 0) {
        AutoScrollLabel *rollLabel = [[AutoScrollLabel alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, 40)];
        ...
        rollLabel.text = adbannerStr;
        [self addSubview:rollLabel];
        self.labelBanner = rollLabel;
    }else{
        [self.labelBanner stopScroll];
        [self.labelBanner removeFromSuperview];
        self.labelBanner = nil;
    }
    [self setViewFrame];
}
...
- (void)adBannerisScroll:(BOOL)scroll{
    if (scroll) {
        [self setAdbannerStr:_adbannerStr];
    }else{
        [self.labelBanner stopScroll];
        [self.labelBanner removeFromSuperview];
        self.labelBanner = nil;
...
- (void)setWeekList:(NSArray *)weekList {
    ...
    for (int i = 0; i < weekList.count; i++) {
        ...
        if (model.isPrivate && !model.isPay) {
        ...
        int num = model.looks;
        if (num > 10000) {
            lookLabel.text = [NSString stringWithFormat:@"%.2f万观看",num/10000.0];
        }else{
            lookLabel.text = [NSString stringWithFormat:@"%d观看",num];
        }
...
- (void)setViewFrame{
    ...
    if (self.adbannerStr.length > 0) {
        self.labelBanner.y = (y+10);
        y = self.self.labelBanner.maxY;
    }
...
- (CGSize)sizeForPageInFlowView:(NewPagedFlowView *)flowView {
    return CGSizeMake(kScreenWidth,  kScreenWidth/3.0 + 50);
}
...
-(void)didScrollToPage:(NSInteger)pageNumber inFlowView:(NewPagedFlowView *)flowView {
    //    NSLog(@"cell == %@",flowView.reusableCells);
...
-(void)dealloc{
    ...
    [_labelBanner stopScroll];
    [_labelBanner removeFromSuperview];
    _labelBanner = nil;
}
