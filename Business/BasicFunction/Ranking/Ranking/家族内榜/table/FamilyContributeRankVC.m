...
@interface FamilyContributeRankVC ()<UITableViewDelegate,UITableViewDataSource,TotalRankHeaderDelegate>
...
@implementation FamilyContributeRankVC
...
- (void)loadData:(BOOL)refresh{
    ...
    [HttpApiChatFamilyController getAppFamilyUserRankVO:self.familyId pageIndex:pageIndex pageSize:pageSize queryType:1 callback:^(int code, NSString *strMsg, NSArray<AppFamilyUserRankVOModel *> *arr) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (model) {
        NSString *numS = [NSString stringWithFormat:@"%.0f",model.familyTotalContribution];
...
- (void)showUserInfo:(id)userInfoM userHeaderView:(nonnull RankUserInfoHeaderView *)headerV{
    if ([userInfoM isKindOfClass:[AppFamilyUserRankVOModel class]]) {
        ...
        NSString *numS = [NSString stringWithFormat:@"%.0f",dtoM.familyTotalContribution];
