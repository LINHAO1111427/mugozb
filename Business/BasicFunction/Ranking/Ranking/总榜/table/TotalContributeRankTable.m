...
@interface TotalContributeRankTable ()<UITableViewDelegate,UITableViewDataSource,TotalRankHeaderDelegate>
...
@implementation TotalContributeRankTable
...
- (void)loadData:(BOOL)refresh isMine:(BOOL)isMine{
    kWeakSelf(self);
    [HttpApiAPPFinance contributionList:isMine?[ProjConfig userId]:0 findType:1 sex:-1 showId:@"" type:(int)self.showType callback:^(int code, NSString *strMsg, NSArray<RanksDtoModel *> *arr) {
        if (isMine) {
            if (code == 1) {
...
- (TotalRankHeader *)header{
    if (!_header) {
        ...
        _header.bgImageV.image = [UIImage imageNamed:@"icon_rank_contribute_header"];
...
- (RankTable *)tableView{
    if (!_tableView) {
        _tableView = [[RankTable alloc]initWithFrame:CGRectMake(0, -kNavBarHeight, kScreenWidth, kScreenHeight-70+kNavBarHeight-kSafeAreaBottom) style:UITableViewStyleGrouped headerHeight:self.header.height-80];
