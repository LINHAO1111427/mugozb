...
@interface UserAttentionLiveVC ()<
UICollectionViewDelegate,
UICollectionViewDataSource,
HomeCarouselHeaderViewDelegate,
UIScrollViewDelegate,CollectionWaterfallLayoutDelegate
>
...
@implementation UserAttentionLiveVC
...
- (void)loadData:(BOOL)isRefresh{
    kWeakSelf(self);
    [self loadSliderData:2 callBack:^(BOOL success) {
...
- (void)loadListData:(BOOL)isRefresh{
    ...
    kWeakSelf(self);
    [HttpApiHome lobbyAttention:self.liveType>0?self.liveType:-1 page:self.page pageSize:kPageSize callback:^(int code, NSString *strMsg, PageInfo *pageInfo, NSArray<AppHomeHallDTOModel *> *arr) {
        if (code == 1) {
...
- (void)loadSliderData:(int)type callBack:(void(^)(BOOL success))callback{
    kWeakSelf(self);
    [HttpApiAppLogin adslist:31 type:31 callback:^(int code, NSString *strMsg, NSArray<AppAdsModel *> *arr) {
...
- (void)roomJump:(AppHomeHallDTOModel*)model{
    [HttpApiHome getHomeDataInfo:model.liveType roomId:model.roomId callback:^(int code, NSString *strMsg, AppHomeHallDTOModel *model) {
        if (code == 1) {
        }else{
            [SVProgressHUD showInfoWithStatus:strMsg];
        }
...
- (CGFloat)waterFallLayout:(CollectionWaterfallLayout *)waterFallLayout heightForItemAtIndexPath:(NSInteger)indexPath itemWidth:(CGFloat)itemWidth{
    ...
    if (path.row < self.infoMuArray.count) {
        AppHomeHallDTOModel *model = self.infoMuArray[path.row];
        liveType = model.liveType;
        height = [self getCellHeightForLiveType:liveType];
    }
    return height;
}
...
- (CGFloat)getCellHeightForLiveType:(int)liveType{
