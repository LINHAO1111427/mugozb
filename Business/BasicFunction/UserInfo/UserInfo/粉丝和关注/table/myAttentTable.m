...
@interface myAttentTable()<UITableViewDelegate,UITableViewDataSource>
...
- (void)creatUI{
    ...
    empty.titleL.text = kLocalizationMsg(@"你还没有关注其他人");
...
- (void)loadAttentionData{
    kWeakSelf(self);
    [HttpApiChatMsgController getAppBookShowInfoList:4 callback:^(int code, NSString *strMsg, NSArray<AppBookShowInfoVOModel *> *arr) {
        if (code == 1) {
...
- (void)relationTop:(AppBookShowInfoVOModel *)model isTop:(BOOL)isTop{
    kWeakSelf(self);
    if (isTop) {
        [HttpApiChatMsgController addChatTopRecord:4 topType:model.groupType topUGID:0 topUserOrGorupId:model.userOrGroupId callback:^(int code, NSString *strMsg, NSArray<ChatTopRecordVOModel *> *arr) {
            if (code == 1) {
                [weakself loadData:YES];
            }else{
                [SVProgressHUD showInfoWithStatus:strMsg];
        ...
        [HttpApiChatMsgController delChatTopRecord:(int)model.topId topPosition:4 callback:^(int code, NSString *strMsg, NSArray<ChatTopRecordVOModel *> *arr) {
            if (code == 1) {
                [weakself loadData:YES];
            }else{
                [SVProgressHUD showInfoWithStatus:strMsg];
