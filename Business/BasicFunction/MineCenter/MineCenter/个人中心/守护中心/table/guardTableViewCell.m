...
@implementation guardTableViewCell
...
- (void)creatUI{
    ...
    CGFloat big_imageW = 0;
    CGFloat small_imageW = 0;
    if (self.type == 0) {
        big_imageW = 86;
        small_imageW = 40;
    }else{
    ...
    bigImageV.contentMode = UIViewContentModeScaleAspectFill;
    ...
    smallImageV.contentMode = UIViewContentModeScaleAspectFill;
    ...
    if (self.type == 1) {
        [self.contentView bringSubviewToFront:bigImageV];
    }
    ...
    //过期
    CGFloat renew_h = 80*68/150.0;
    UIButton *renewBtn = [[UIButton alloc]initWithFrame:CGRectMake(kScreenWidth-85, height-renew_h, 80, renew_h)];
    [renewBtn setBackgroundImage:[UIImage imageNamed:@"icon_mine_guard_renew"] forState:UIControlStateNormal];
    [renewBtn addTarget:self action:@selector(renewBtnClcik:) forControlEvents:UIControlEventTouchUpInside];
    renewBtn.hidden = YES;
    self.renewBtn = renewBtn;
    [self.contentView addSubview:renewBtn];
}
- (void)setGuardModel:(GuardUserVOModel *)guardModel{
    _guardModel = guardModel;
    // 时间
    NSString *dayStr = [NSString stringWithFormat:kLocalizationMsg(@"%@%lld 天"),self.type==0?kLocalizationMsg(@"已守护"):kLocalizationMsg(@"守护我"),guardModel.guardDay];
    ...
    if (self.type == 0) {//守护别人
        self.titleL.text = [NSString stringWithFormat:@"%@",guardModel.anchorName];
        [self.bigImageV sd_setImageWithURL:[NSURL URLWithString:guardModel.anchorIdImg] placeholderImage:[ProjConfig getAppIcon]];
        [self.smallImageV sd_setImageWithURL:[NSURL URLWithString:guardModel.userHeadImg] placeholderImage:[ProjConfig getAppIcon]];
    }else{//被人守护
...
- (void)renewBtnClcik:(UIButton *)btn{
    if (self.deleagte && [self.deleagte respondsToSelector:@selector(guardTableViewCellRenewBtnClickWith:)]) {
        [self.deleagte guardTableViewCellRenewBtnClickWith:self.guardModel];
    }
}
@end
