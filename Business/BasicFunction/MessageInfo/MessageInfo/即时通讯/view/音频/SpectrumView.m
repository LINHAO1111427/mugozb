...
- (void)setup {
   ...
   // NSLog(@"过滤文字setup"));
    ...
    self.itemColor = [UIColor grayColor];

    self.middleInterval = 30.f;
    
    self.timeLabel = [[UILabel alloc]init];
    self.timeLabel.text = @"01:59";
...
#pragma mark - setter
- (void)setHiddenLabel:(BOOL)hiddenLabel{
...
- (void)setItemLevelCallback:(void (^)(void))itemLevelCallback {
   // NSLog(@"过滤文字setItemLevelCallback"));
...
- (void)updateItems {
    //NSLog(@"过滤文字updateMeters"));

    UIGraphicsBeginImageContext(self.frame.size);

    int lineOffset = self.lineWidth * 2.f;
    
    int leftX = (self.itemWidth - self.middleInterval + self.lineWidth) / 2.f - 20;
    int rightX = (self.itemWidth + self.middleInterval - self.lineWidth) / 2.f + 20;
